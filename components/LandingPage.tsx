...
const LandingPage: React.FC<LandingPageProps> = ({ onLogin, isLoading }) => {
  ...
  return (
    <div className={`min-h-screen font-sans selection:bg-violet-500 selection:text-white overflow-x-hidden ${theme === 'light' ? 'bg-gray-50 text-gray-900' : 'bg-slate-950 text-slate-50'}`}>

      {/* Navigation */}
      <nav className={`fixed w-full z-50 backdrop-blur-md border-b ${theme === 'light' ? 'bg-white/80 border-gray-200' : 'bg-slate-950/80 border-slate-800'}`}>
      ...
      <div className="relative pt-32 pb-20 lg:pt-48 lg:pb-32 px-6">
        ...
        <div className="max-w-4xl mx-auto text-center relative z-10">
          ...
          <p className={`mt-6 text-sm ${theme === 'light' ? 'text-gray-500' : 'text-slate-500'}`}>Free to start • No credit card required</p>
        </div>
      </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          ...
          <div className={`group relative rounded-3xl p-8 border transition-colors overflow-hidden ${theme === 'light' ? 'bg-white border-emerald-200 hover:border-emerald-300 hover:bg-emerald-50/30' : 'bg-slate-900/50 border-emerald-500/20 hover:bg-slate-900 hover:border-emerald-500/50'}`}>
            ...
            <div className="relative z-10">
              ...
              <ul className={`space-y-3 ${theme === 'light' ? 'text-emerald-800' : 'text-emerald-100/80'}`}>
                <li className="flex items-center gap-3">
          ...
          {/* Stash Card */}
          <div className={`group relative rounded-3xl p-8 border transition-colors overflow-hidden ${theme === 'light' ? 'bg-white border-amber-200 hover:border-amber-300 hover:bg-amber-50/30' : 'bg-slate-900/50 border-amber-500/20 hover:bg-slate-900 hover:border-amber-500/50'}`}>
            ...
            <div className="relative z-10">
              ...
              <ul className={`space-y-3 ${theme === 'light' ? 'text-amber-800' : 'text-amber-100/80'}`}>
                <li className="flex items-center gap-3">
      ...
      {/* Footer */}
      <footer className={`border-t py-12 text-center text-sm ${theme === 'light' ? 'border-gray-200 bg-white text-gray-500' : 'border-slate-800 bg-slate-950 text-slate-500'}`}>
        <p>© {new Date().getFullYear()} VibeForce Inc. All rights reserved.</p>
        <div className="mt-4 flex justify-center gap-6">
          <a href="#" className={`transition-colors ${theme === 'light' ? 'hover:text-gray-800' : 'hover:text-slate-300'}`}>Privacy</a>
          <a href="#" className={`transition-colors ${theme === 'light' ? 'hover:text-gray-800' : 'hover:text-slate-300'}`}>Terms</a>
          <a href="#" className={`transition-colors ${theme === 'light' ? 'hover:text-gray-800' : 'hover:text-slate-300'}`}>Twitter</a>
        </div>
      </footer>
