    ...
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          {/* Date Selector & Mood */}
          <div
            className={`rounded-xl p-6 border ${theme === 'light' ? 'bg-white border-purple-200' : 'bg-slate-800 border-purple-500/30'}`}
          >
            ...
            <div>
              <label
                className={`block text-sm font-medium mb-3 ${theme === 'light' ? 'text-gray-700' : 'text-slate-300'}`}
              >
                How are you feeling?
              </label>
              <div className="flex gap-2">
                {Object.entries(MOODS).map(([key, { emoji, label }]) => (
                  ...
                  >
                    ...
                    <div
                      className={`text-xs mt-1 ${theme === 'light' ? 'text-gray-600' : 'text-slate-400'}`}
                    >
                      {label}
                    </div>
                  </button>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            >
              <h3
                className={`text-sm font-semibold mb-3 flex items-center gap-2 ${theme === 'light' ? 'text-gray-900' : 'text-white'}`}
              >
                <span>🎉</span> Wins
              </h3>
              <div className="space-y-2 mb-3">
                {wins.map((win, i) => (
                  <div
                    key={i}
                    className={`text-sm p-2 rounded ${theme === 'light' ? 'bg-green-50 text-green-800' : 'bg-green-500/10 text-green-300'}`}
                  >
                    {win}
                  </div>
                ))}
              ...
              <div className="flex gap-2">
                ...
                />
                <button onClick={addWin} className="text-green-500 hover:text-green-400">
                  +
                </button>
              </div>
            </div>
            ...
            {/* Learnings */}
            <div
              className={`rounded-xl p-6 border ${theme === 'light' ? 'bg-white border-blue-200' : 'bg-slate-800 border-blue-500/30'}`}
            >
              <h3
                className={`text-sm font-semibold mb-3 flex items-center gap-2 ${theme === 'light' ? 'text-gray-900' : 'text-white'}`}
              >
              ...
              <div className="space-y-2 mb-3">
                {learnings.map((learning, i) => (
                  <div
                    key={i}
                    className={`text-sm p-2 rounded ${theme === 'light' ? 'bg-blue-50 text-blue-800' : 'bg-blue-500/10 text-blue-300'}`}
                  >
                    {learning}
                  </div>
                ))}
              ...
              <div className="flex gap-2">
                ...
                />
                <button onClick={addLearning} className="text-blue-500 hover:text-blue-400">
                  +
                </button>
              </div>
            </div>
        ...
        <div className="space-y-4">
          ...
          {entries.slice(0, 10).map(entry => (
            ...
            >
              <div className="flex items-center justify-between mb-2">
                ...
                <span
                  className={`text-xs ${theme === 'light' ? 'text-gray-600' : 'text-slate-400'}`}
                >
              ...
              <p
                className={`text-sm line-clamp-2 ${theme === 'light' ? 'text-gray-600' : 'text-slate-400'}`}
              >
                {entry.content}
              </p>
          ...
          {entries.length === 0 && (
            <div
              className={`text-center py-8 border-2 border-dashed rounded-lg ${theme === 'light' ? 'border-gray-300' : 'border-slate-700'}`}
            >
