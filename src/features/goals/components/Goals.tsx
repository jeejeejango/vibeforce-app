...
const Goals: React.FC<GoalsProps> = ({ user, goals, setGoals }) => {
  ...
  const handleSaveEdit = async () => {
    ...
    const updatedGoals = goals.map(g => (g.id === selectedGoal.id ? { ...g, ...updates } : g));
    ...
    <div className="p-6 max-w-7xl mx-auto">
        ...
        {goals.map(goal => {
          ...
          return (
            <div
              ...
              className={`group relative rounded-xl p-6 border cursor-pointer transition-all overflow-hidden ${
                theme === 'light'
            ...
            >
              ...
              <div className="relative z-10">
                ...
                {goal.description && (
                  <p
                    className={`text-sm mb-4 line-clamp-2 ${theme === 'light' ? 'text-gray-600' : 'text-slate-400'}`}
                  >
                    {goal.description}
                  </p>
                )}
                ...
                <div className="flex items-center gap-4 mb-4">
                  ...
                  <div>
                    ...
                    <div
                      className={`text-xs mt-1 ${daysLeft > 0 ? 'text-yellow-500' : 'text-red-500'}`}
                    >
      ...
      {selectedGoal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div
            className={`max-w-2xl w-full rounded-2xl p-8 max-h-[90vh] overflow-y-auto ${
              ...
              {isEditing ? (
                ...
                <div>
                  ...
                  <div className="flex gap-2 mt-3">
                    <span
                      className={`inline-block px-3 py-1 text-xs rounded-full ${theme === 'light' ? 'bg-yellow-100 text-yellow-800' : 'bg-yellow-500/20 text-yellow-300'}`}
                    >
                      {selectedGoal.category}
                    </span>
                    <span
                      className={`inline-block px-3 py-1 text-xs rounded-full ${theme === 'light' ? 'bg-gray-100 text-gray-700' : 'bg-slate-700 text-slate-300'}`}
                    >
      ...
      {showCreateModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div
            className={`max-w-lg w-full rounded-2xl p-8 ${theme === 'light' ? 'bg-white' : 'bg-slate-800'}`}
          >
            <h2
            ...
            <div className="space-y-4">
              <div>
                <label
                  className={`block text-sm font-medium mb-2 ${theme === 'light' ? 'text-gray-700' : 'text-slate-300'}`}
                >
                  Goal Title *
                </label>
                <input
