...
const Focus: React.FC<FocusProps> = ({ user, todoLists }) => {
  ...
  const allTasks = todoLists.flatMap(list => list.tasks.filter(t => t.status !== 'done'));
  ...
  const handleSessionComplete = () => {
    ...
    const audio = new Audio(
      'data:audio/wav;base64,UklGRnoGAABXQVZFZm10IBAAAAABAAEAQB8AAEAfAAABAAgAZGF0YQoGAACBhYqFbF1fdJivrJBhNjVgodDbq2EcBj+a2/LDciUFLIHO8tiJNwgZaLvt559NEAxQp+PwtmMcBjiR1/LMeSwFJHfH8N2QQAoUXrTp66hVFApGn+DyvmwhBTGH0fPTgjMGHm7A7+OZQRE='
    );
    audio.play().catch(() => {});
  ...
  const progress =
    ((SESSION_DURATIONS[sessionType] - timeLeft) / SESSION_DURATIONS[sessionType]) * 100;
    ...
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div
            className={`rounded-xl p-6 border ${theme === 'light' ? 'bg-white border-gray-200' : 'bg-slate-800 border-slate-700'}`}
          >
          ...
          {/* Circular Timer */}
          <div
            className={`rounded-xl p-8 border ${theme === 'light' ? 'bg-white border-gray-200' : 'bg-slate-800 border-slate-700'}`}
          >
            <div className="flex flex-col items-center">
              ...
              <div className="relative w-64 h-64 mb-8">
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <span
                    className={`text-6xl font-bold ${theme === 'light' ? 'text-gray-900' : 'text-white'}`}
                  >
                    {formatTime(timeLeft)}
                  </span>
                </div>
          ...
          {sessionType === 'work' && (
            ...
            >
              <h3
                className={`text-sm font-semibold uppercase tracking-wider mb-3 ${theme === 'light' ? 'text-gray-600' : 'text-slate-400'}`}
              >
                Working On (Optional)
              </h3>
              <select
        ...
        <div className="space-y-6">
            ...
            <div className="space-y-4">
              <div>
                <div className="flex justify-between items-end mb-2">
                  ...
                  <span
                    className={`text-3xl font-bold ${theme === 'light' ? 'text-blue-600' : 'text-blue-400'}`}
                  >
                    {completedSessions}
                  </span>
                </div>
                <div
                  className={`h-2 rounded-full overflow-hidden ${theme === 'light' ? 'bg-gray-200' : 'bg-slate-700'}`}
                >
                  <div
                ...
                <p
                  className={`text-xs mt-1 ${theme === 'light' ? 'text-gray-500' : 'text-slate-500'}`}
                >
                  Goal: 8 sessions
                </p>
              </div>
          ...
          {/* Tips */}
          <div
            className={`rounded-xl p-6 border ${theme === 'light' ? 'bg-blue-50 border-blue-200' : 'bg-blue-900/20 border-blue-800/50'}`}
          >
            ...
            <p className={`text-sm ${theme === 'light' ? 'text-blue-900' : 'text-blue-200'}`}>
              Take breaks seriously! Your brain needs rest to maintain peak performance. Stand up,
              stretch, and hydrate.
            </p>
          </div>
          ...
          >
            ...
            <ol
              className={`space-y-2 text-sm ${theme === 'light' ? 'text-gray-600' : 'text-slate-400'}`}
            >
