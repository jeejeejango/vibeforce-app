...
export const generateTasksFromNaturalLanguage = async (prompt: string): Promise<string[]> => {
  const ai = getAIClient();
  if (!ai) return [];
...
export const getSmartProductivityTip = async (context: string): Promise<string> => {
  const ai = getAIClient();
